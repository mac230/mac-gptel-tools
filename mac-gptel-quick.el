...
(defvar gptel-quick--last-response nil
  "Most recent response string from `gptel-quick'.")
...
quick actions on the popup."
  (pcase response
    ...
    ((pred stringp)
     (setq gptel-quick--last-response response)
     (pcase-let ((`(,query ,count ,pos) (plist-get info :context)))
       ...
       (cl-flet ((clear-response () (interactive)
                 ...
                 (insert-response () (interactive)
                   (with-current-buffer (window-buffer (selected-window))
                     (insert response))
                   (message "Inserted gptel-quick response."))
         ...
         (set-transient-map
          (let ((map (make-sparse-keymap)))
            ...
            (define-key map (kbd "m") #'more-response)
            (define-key map [remap kill-ring-save] #'copy-response)
            (define-key map (kbd "i") #'insert-response)
