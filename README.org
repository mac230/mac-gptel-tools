...
(gptel-context-remove-all)
(setq gptel-quick-backend (gptel-get-backend "mac-claude")
...
** Add a New Model
#+BEGIN_SRC emacs-lisp :eval no
;; basic approach - add to existing set of backends:
(push 'gpt-5
      (gptel-backend-models (gptel-get-backend "ChatGPT")))

;; custom approach - define capabilities:
(gptel-make-openai "gpptest"
  :stream t
  :key (auth-source-pick-first-password :host "api.openai.com")
  :models 
  '((gpt-5
     :description "GPT5 from openai"
     :capabilities (tool-use json media)
     :mime-types ("image/png" "image/jpeg" "image/webp" "image/heic" "image/heif"
                  "application/pdf" "text/plain" "text/csv" "text/html")))
  )
