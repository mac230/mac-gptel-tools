    ...
    (if (not pdf-buffer)
      ...
      (progn
        ...
        (if (eq system-type 'darwin)
            ...
            "screencapture -T 1 /tmp/shot.png")
        (start-process-shell-command "" nil
...
** Getting Information on References in a pdf as a Popup Tooltip
I use the following function to have emacs display the full citation for a paper
within a pdf using 'pdf-tools' and 'gptel'.  The function prompts the user for a
string that identifies the in-text citation.  It could be a reference number
(e.g., "120") or name / year citation (e.g., "Collins 2020").  'pdf-tools' then
prompts the user to pick a link to follow and sends the text on the associated
page to a blank buffer.  That text output is fed to gptel with instructions to
display the citation as a popup tooltip.  

#+BEGIN_SRC emacs-lisp 
(defun pop-ref-with-gptel ()
  "Provide the title of a work referenced in a pdf using 'pdf-tools' and 'gptel'."
  (interactive)
  (let ((current-window (selected-window))
    ...
    (setq ref-number (read-string "reference number: "))
    (global-data-entry-mode -1)
    
    ;; find the pdf buffer in my windows:
    (dolist (w (window-list))
      (with-current-buffer (window-buffer w)
        (when (eq major-mode 'pdf-view-mode)
          (setq pdf-buffer (current-buffer)
                pdf-window (get-buffer-window pdf-buffer)
                pdf-page (pdf-view-current-page)))))
    ...
    ;; 
    (select-window pdf-window)
    ;; instead of pre-specifying a link
    ;; just highlight all links and let me
    ;; pick.  this requires the same amount
    ;; of typing (two keys) and happens on
    ;; home row instead of numbers:
    (pdf-links-action-perform
     (pdf-links-read-link-action "Follow link: "))
    (setq ref-text (get-text-to-process))
    (select-window current-window)

    (with-current-buffer (get-buffer-create "ref_info")
      (delete-region (point-min) (point-max))
      (goto-char (point-min))
      (insert (concat "what is reference "
                      ref-number
                      "\n Just the reference text, no extra content.\n"
                      ref-text
                      "\n"
                      ))
      (gptel-request (buffer-substring-no-properties (point-min) (point-max))
        :system (funcall gptel-quick-system-message 100)
        :context (list (buffer-substring-no-properties (point-min) (point-max))
                       100
...
;; prompt from minibuffer, echo response in minibuffer
;; note: context added via gptel menu is not accessible using this command
(defun gptel-prompt-from-minibuffer-echo-message (prompt)
...
;; test prompt:
;; @rreader read the help page for the R function "xyplot"
;; and provide a slightly altered version of one of the plot calls
#+END_SRC
...
(defun gptel-add-buffers-headings-to-context ()
  ...
  (let ((entries)
        ...
        (files '(
               ...
               "latex_notes.org"
               "org_notes.org"
               "figure_examples_I_like.org"
               "degradation_papers.org"
               "genetics_papers.org"
               "proteomics_papers.org"
               "genetics_papers.org"
               "agenda.org"
               "scratch-pad.org"
               "writing.org"
    ...
    (when nas-ssh-connection
      (dolist (f files)
        (when (not (bufferp (get-buffer f)))
          (when (file-exists-p (concat "/ssh:mac:~/emacs/org/" f))
            (find-file-noselect (concat "/ssh:mac:~/emacs/org/" f))))
        ))
    ...
    (setq contents
          (mapcar (lambda (e) (substring-no-properties (car e)))
                  entries)
          buffers
          (mapcar (lambda (f) (buffer-name (marker-buffer (cdr f))))
                    entries))
    ...
    (with-current-buffer (get-buffer-create "this_test")
      (delete-region (point-min) (point-max))
      (let ((bs buffers) (cs contents))
        (while (and bs cs)
          (insert (format "buffer: %s header: %s\n" (car bs) (car cs)))
          (setq bs (cdr bs) cs (cdr cs))))
      (mark-whole-buffer)
      (gptel-context-add)
)))
...
(gptel-make-tool
 ...
 :name "get_org_heading_switch_to_buffer"
...
(gptel-make-preset
  ...
  :tools        '("get_org_heading_switch_to_buffer"))

;; @org-buffer-finder2 find my notes on making pngs from R plot pdfs 
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :eval no
(load-file "~/.emacs.d/elpa/gptel-0.9.8.5/gptel-context.el")
(org-babel-load-file "/ssh:mac@mac:~/emacs/mac-gptel-tools/README.org")
(gptel-context-remove-all)

(let ((qfile "/ssh:mac@mac:~/emacs/mac-gptel-tools/mac-gptel-quick.el"))
  (when (file-exists-p qfile)
    (load-file qfile)
    (message "loaded your gptel-quick version")
    ))


(setq gptel-quick-backend (gptel-get-backend "ctest")
      gptel-quick-model 'claude-haiku-4-5
      ;; have the response stay longer than default of 10
      gptel-quick-timeout 20)


;; -----
;; add latest claude models:

;; 1. opus (most powerful):
(cl-pushnew 'claude-opus-4-7
            (gptel-backend-models
             (gptel-get-backend "mac-claude")))

;; 2. sonnet (balance speed / power): 
(cl-pushnew 'claude-sonnet-4-6
            (gptel-backend-models
             (gptel-get-backend "mac-claude")))

;; 3. haiku (fastest):
(cl-pushnew 'claude-haiku-4-5
            (gptel-backend-models
             (gptel-get-backend "mac-claude")))
...
(push 'gemini-flash-lite-latest
      (gptel-backend-models (gptel-get-backend "gemini")))

(with-eval-after-load 'gptel
  (setq gptel-directives
        (append gptel-directives
                '(("pdf_spray_extractor" . "you will receive text that has been extracted from a pdf.  the user would like to use this text for a speed reading application.  your task is to improve the clarity of the text by removing extraneous content.  the content you will remove may include references (numbers or last names (e.g., 'collins et al.')), figure captions, journal names, journal volumes, years, page numbers, and the article title.  you are only to return the cleaned text - no 'I'll process the text for you' or 'if you provide more text I can extract more' or similar messaging - just provide the cleaned text.")))))

(defun spray-get-pdf-text ()
  "Send PROMPT using DIRECTIVE-NAME just for this call."
  (interactive)
  (let ((gptel--system-message
         (cdr
          (assoc
           "pdf_spray_extractor"
           gptel-directives)))
        (prompt (get-text-to-process)))
    (when prompt
      (gptel-request prompt)
      (if (yes-or-no-p "enter spray mode?")
          (spray-mode)
        (message "not entering spray mode")
      ))
    ))
...
* Adding New Models
** gptel: Adding New Models to gptel
#+BEGIN_SRC emacs-lisp :results silent 
;; want to do it this way so the capabilities,
;; esp. reading pdfs get added to each model.
(gptel-make-anthropic "ctest"
  :stream t
  :key (auth-source-pick-first-password :host "api.anthropic.com")
  :models 
  '((claude-opus-4-7
    ...
    (claude-opus-4-8
     ...
     :mime-types ("image/png" "image/jpeg" "image/webp" "image/heic" "image/heif"
                  "application/pdf" "text/plain" "text/csv" "text/html"))
    (claude-sonnet-4-6
     :description "speed / power balance claude model"
     :capabilities (tool-use json media)
     :mime-types ("image/png" "image/jpeg" "image/webp" "image/heic" "image/heif"
                  "application/pdf" "text/plain" "text/csv" "text/html"))
    (claude-haiku-4-5
...
;; the following adds a new model to a backend
...
;;                   "application/pdf" "text/plain" "text/csv" "text/html"))
;;             (gptel-backend-models
;;              (gptel-get-backend "ctest")))

#+END_SRC

** chatGPT-shell: Adding New Models to chatGPT-shell
#+BEGIN_SRC emacs-lisp :results silent 
;; have to do this on a per-provider basis:
(cl-pushnew (chatgpt-shell-anthropic--make-model
              :version "claude-opus-4-7"
              :short-version "opus-4.7"
              :token-width 4
              :thinking-budget-min 1024
              :reasoning-effort-selector
              #'chatgpt-shell-anthropic-reasoning-effort-selector
              :max-tokens 50000
...
#+BEGIN_SRC emacs-lisp :results silent 
;; the set of available system prompts can be seen here:
;; or just describe the variable 'chatgpt-shell-system-prompts':
;; (princ chatgpt-shell-system-prompts (current-buffer))

;; ((mac default . you are a helpful assistant who responds concisely (no sure I'll look that up or similar).  you'll be asked to provide mathematical formulas.  when you do so, you are not to use latex, just regular fonts / symbols))

;; here's the original value:
;; (("tl;dr" .
;;   "Be as succint but informative as possible and respond in tl;dr form to my queries")
;;  ("General" .
...
;;                         You treat their time as precious. You do not repeat obvious
;;                         things, including their query.
;;                         You are as concise as possible in responses.

;; you can add your own prompt as follows:
(add-to-list 'chatgpt-shell-system-prompts
             '("mac default" . "you are a helpful assistant who responds
               concisely (so don't waste the user's time with responses like
               'sure I'll look that up' or similar).
               you'll be asked to provide mathematical formulas.
               when you do so, you are not to use latex, just plain-text symbols.
               Be as concise as possible in responses.
               don't use aggressive markdown formatting
               (e.g., excessive bolding).  The user is working in emacs and relies
               on org-mode so format accordingly and wrap code in emacs org-mode blocks.
               Don't use org-mode headings or boldings (they're distracting), instead use
               structured text heading styles that incorporate '-', '/', etc..."
))

;; then, select it with:
;; M-x chatgpt-shell-swap-system-prompt

;; note that calling the 'chatgpt-shell-load-awesome-prompts' adds > 60e3
;; prompts as of 2026.04.15 and massively slows down chatgpt-shell.  if
;; load these by mistake, you can set 'chatgpt-shell-system-prompts' to nil
;; and start over again, which seems to resolve the issue
#+END_SRC

** gptel System Prompt and Re-Write Directives
#+BEGIN_SRC emacs-lisp :results silent 
...
(setf (alist-get 'rewrite gptel-directives)
      "Fix typos, capitalization errors, punctuation errors and always leave two spaces after periods")
